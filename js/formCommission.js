...
function formCommission(unId) {
	...
	var tabPanelCommission = new Ext.TabPanel({
		 activeItem:0,
...
//				value: 'Display field <span style="color:green;">value</span>',
				...
				name: 'COM_Comment',
				anchor: '-12 -120'  // ou '95% -96'
			},{
				xtype:'panel',
				border: false,
				html: '<a href = "./php/uploadDocGed.php?idAction=' + unId + '" TARGET="_blank">Chargement des fichiers par lots</a>'
			}]
		}, this.gridPanelDroit
		]
	});
	...
	this.formulaire = new Ext.form.FormPanel({
		...
		bbar : [{
			xtype: 'tbtext',
			text: unId > 0 ? (href.split('?'))[0] + "?tb=com&id=" + unId : ""
		}]
	...
	if( unId > 0 ) {
		this.formulaire.load({
			...
			success : function(form, action) {
				...
				form.findField('taille').setValue( Math.round( 100 * obj.data.taille / 1024 / 1024 )/100 + ' Mo');
			}
		});
